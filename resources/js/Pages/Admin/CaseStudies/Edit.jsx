...
export default function CaseStudiesEdit({ caseStudy, categories = [] }) {
    ...
    return (
        <AdminLayout>
            <Head title={`Edit — ${caseStudy.title}`} />
            <PageHeader
                title={`Edit: ${caseStudy.title}`}
                breadcrumbs={[
            ...
            <form onSubmit={e => { e.preventDefault(); post(route('admin.case-studies.update', caseStudy.id), { forceFormData: true }); }} encType="multipart/form-data">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                    <div className="lg:col-span-2 space-y-5">
                        ...
                        <div className="card p-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <Field label="Nilai Metric"><Input value={data.metric_value} onChange={e => setData('metric_value', e.target.value)} placeholder="+25%" /></Field>
                                <Field label="Label Metric"><Input value={data.metric_label} onChange={e => setData('metric_label', e.target.value)} placeholder="Energy Saved" /></Field>
                            </div>
                        </div>
                        ...
                        <div className="card p-6">
                            ...
                            {caseStudy.image && (
                                <div className="mb-3 flex items-center gap-3 p-3 rounded-lg border border-border bg-bg-hover">
                                    <img src={`/storage/${caseStudy.image}`} alt="" className="w-16 h-12 object-cover rounded" />
                                    <p className="text-xs text-text-muted">Gambar saat ini</p>
                                </div>
                            )}
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="card p-5 flex flex-col gap-3">
                            <button type="submit" disabled={processing}
                                ...
                                {processing ? <><svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg> Menyimpan...</> : <><Save size={15} /> Simpan Perubahan</>}
                            </button>
                            <Link href={route('admin.case-studies.index')}
